...
def mix_structure(vecR, list_pos1, list_pos2, ratio, head, tail, filename_out):
    ...
    for p1, p2 in zip(list_pos1, list_pos2):
        ...
        element1 = p1["species"].translate({None: digits}) # my change
...
if (os.path.exists(filename_scf)):
...
# (vecR2, list_pos2), prog = read_cell_and_pos_auto("../relax-cdftdn1/relax")

# For VASP
(vecR, list_pos1), prog = read_cell_and_pos_auto("../relax-gs/CONTCAR")
(vecR2, list_pos2), prog = read_cell_and_pos_auto("../relax-cdftup1/CONTCAR")
...
#for ratio in (0, 0.1667, 0.3333, 0.5, 0.6667, 0.8333, 1.00):
...
for ratio in partition: #(0, 0.1667, 0.3333, 0.5, 0.6667, 0.8333, 1.00):
    dirname = "ratio-%.4f" % ratio
    if (not os.path.exists(dirname)):
        os.makedirs(dirname)
    if (os.path.exists(os.path.join(dirname, "POSCAR"))):
        continue
    mix_structure(vecR, list_pos1, list_pos2, ratio, head, tail, os.path.join(dirname, "POSCAR"))
