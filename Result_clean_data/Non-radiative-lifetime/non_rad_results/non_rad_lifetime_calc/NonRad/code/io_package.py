...
def read_cell_and_pos_auto(arg):
    ...
    if os.path.exists(arg.replace("scf", "POSCAR")):
        vasp_arg = arg.replace("scf", "POSCAR")
        return read_cell_and_pos_poscar(vasp_arg), "vasp"
    elif os.path.exists(arg.replace("scf", "CONTCAR")):
        vasp_arg = arg.replace("scf", "CONTCAR")
        return read_cell_and_pos_poscar(vasp_arg), "vasp"
...
def read_cell_and_pos_poscar(filename):
    ...
    for atom, n in zip(atoms, natoms):
        for j in range(n):
            list_pos.append({"species": str(atom), "pos": np.asarray([float(x) for x in lines[i].split()])})
            i += 1
