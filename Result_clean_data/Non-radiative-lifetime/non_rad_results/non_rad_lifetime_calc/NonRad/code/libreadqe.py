...
def read_pos_and_etot_ratio(folder=None):
    ''' Function to read the position and energy from a given directory using different programs'''
    ...
    elif not glob.glob("%s/ratio-*/scf.out" % folder) and not glob.glob("%s/ratio-*/OUTCAR" % folder):
        raise FileExistsError("files %s/ratio-*/scf.out or ratio-*/OUTCAR do not exist" % folder)

    elif glob.glob("%s/ratio-*/scf.out" % folder) and not glob.glob("%s/ratio-*/OUTCAR" % folder):
        return read_pos_and_etot_ratio_qe(folder), 'qe'

    elif glob.glob("%s/ratio-*/OUTCAR" % folder) and not glob.glob("%s/ratio-*/scf.out" % folder):
        if not glob.glob("%s/ratio-*/CONTCAR" % folder):
            if not glob.glob("%s/ratio-*/POSCAR" % folder):
                raise FileNotFoundError("files %s/ratio-*/CONTCAR and /ratio-*/POSCAR do not exist" % folder)
            else:
                return read_pos_and_etot_ratio_vasp(folder) , 'vasp'
        else:
            return read_pos_and_etot_ratio_vasp(folder) , 'vasp'

    elif glob.glob("%s/ratio-*/scf.out" % folder) and glob.glob("%s/ratio-*/OUTCAR" % folder):
        raise FileExistsError("Both files %s/ratio-*/scf.out and ratio-*/OUTCAR exist" % folder)


def read_pos_and_etot_ratio_vasp(folder=None):
    '''
    Read ratio-***/OUTCAR and ratio-***/CONTCAR from given directory
...
def read_pos_and_etot_ratio_qe(folder=None):
    '''
...
def read_eig(folder, prog):
    ...
    '''
    if prog == 'qe':
        return read_eig_qe(folder)
    elif prog == 'vasp':
        return read_eig_vasp(folder)
    else:
        raise ValueError("Unknown program %s" % prog)

def read_eig_qe(folder):
    '''
    Function for QE
    Read eigenvalues from save folder
    :return: Array in ik, ispin, ib, 1/2 (for eigenvalues and occupations numbers)
