...
def calc_dE(dir1):
    ...
    '''
    [list_data1, vecR], _ = read_pos_and_etot_ratio(dir1)
    etot1 = None
    etot2 = None
    for x in list_data1:
        if (x["ratio"] == 0):
...
def calc_wif_standalone(dir1, dir2, ix_defect, ix_bandmin, ix_bandmax, de=None):
    ...
    [list_data1, vecR], prog = read_pos_and_etot_ratio(dir1)
    [list_data2, vecR], prog = read_pos_and_etot_ratio(dir2)
...
# Compute dQ
    ...
    ar_eig2_q0 = read_eig(folder1_q0, prog)
    spin1 = 2
    spin2 = 2
...
def calc_wif(dir_i, dir_f, ix_defect, ix_bandmin, ix_bandmax, dQ, de=None, spinname="down"):
    '''
    Compute Wif from a series of SCF calculations; Check Phys. Rev. B 90, 075202(2014) for definition.
    Each SCF calculation must be in ratio-* folder and 
    - in case of using QE: contains a *.save folder as result
    - in case of using VASP: contains WAVECAR or WSWQ files as result
...
# Compute Wifs
    ...
    if prog == 'qe':
        # Extract the folder of q0 from QE
        folder_q0 = get_save_folder(os.path.join(dir_f, get_ratio_folder(0)))
        dE, dic_band_overlap, dic_wif, ixband_wifmax, wif = calc_wif_qe(dir_f, list_data2, folder_q0, ix_defect, ix_bandmin, ix_bandmax, dQ, spin)

    elif prog == 'vasp':
        # Extract the folder of q0 from VASP
        folder_q0 = os.path.join(dir_f, get_ratio_folder(0))
        if os.path.isfile(os.path.join(folder_q0, "WSWQ")):
            print("\n{}Computing Wif from WSWQ files".format(indent))
            dE, dic_band_overlap, dic_wif, ixband_wifmax, wif = calc_wif_from_WSWQ(dir_f, list_data2, folder_q0, ix_defect, ix_bandmin, ix_bandmax, dQ, spin)
        else:
            print("\n{}Computing Wif from WAVECAR files".format(indent))
            dE, dic_band_overlap, dic_wif, ixband_wifmax, wif = calc_wif_from_wavecar(dir_f, list_data2, folder_q0, ix_defect, ix_bandmin, ix_bandmax, dQ, spin)

    return dic_eig_occ, dE, dic_band_overlap, dic_wif, ixband_wifmax, wif

''' Functions to determine Wif from WAVECAR: '''
def wavecar_mult(psi_i, psi_f):
    '''
    Function for VASP WAVECAR (WAVECAR outputs pseudowavefunctions)
    Normalize wavefunctions and product wavefunctions 
    '''
    npsi_i = psi_i / np.sqrt(np.abs(np.vdot(psi_i, psi_i)))
    npsi_f = psi_f / np.sqrt(np.abs(np.vdot(psi_f, psi_f)))
    
    return np.abs(np.vdot(npsi_i, npsi_f))

def calc_wif_from_wavecar(dir_f, list_data2, folder_q0, ix_defect, ix_bandmin, ix_bandmax, dQ, spin):

    from pymatgen.io.vasp.outputs import Wavecar

    initial_wavecar = Wavecar(os.path.join(folder_q0, "WAVECAR"))
    kpoint = 1
    if initial_wavecar.spin == 2:
        psi_i = initial_wavecar.coeffs[spin-1][kpoint-1][ix_defect-1]
    else:
        psi_i = initial_wavecar.coeffs[kpoint-1][ix_defect-1]
    
    num_wave = len(list_data2)
    num_bands = len(list(range(ix_bandmin, ix_bandmax+1)))
    dE = np.zeros(num_bands)
    '''
    Compute eigenvalue differences for dQ=0
    '''
    for i, iband in enumerate(range(ix_bandmin, ix_bandmax+1)):
        if initial_wavecar.spin == 2:
            dE[i] = initial_wavecar.band_energy[spin-1][kpoint-1][iband-1][0] - initial_wavecar.band_energy[spin-1][kpoint-1][ix_defect-1][0]
        else:
            dE[i] = initial_wavecar.band_energy[kpoint-1][iband-1][0] - initial_wavecar.band_energy[kpoint-1][ix_defect-1][0]
    dE = np.abs(dE)

    '''
    Loop through dQ values and bands and compute overlap
    '''
    dic_band_overlap = {iband: [] for iband in range(ix_bandmin, ix_bandmax+1)}
    Q = np.zeros(num_wave+1)
    print("%sGathering :  band ratio" % (indent*2))
    for i, data in enumerate(list_data2):
        ratio = data["ratio"]
        folder2 = os.path.join(dir_f, get_ratio_folder(ratio))
        Q[i] = ratio*dQ  #convert to Q

        if (folder2 is None):
            print("Skip ratio %.4f as missing .save folder %s" % (ratio, os.path.join(dir_f, "ratio-%.4f" % ratio)))
            continue
        
        final_wavecar = Wavecar(os.path.join(folder2, "WAVECAR"))

        for j, iband in enumerate(range(ix_bandmin, ix_bandmax+1)):
            print("%sOverlap: %i  %.4f" % (indent*3, iband, ratio))
            if final_wavecar.spin == 2:
                psi_f = final_wavecar.coeffs[spin-1][kpoint-1][iband-1]
            else:
                psi_f = final_wavecar.coeffs[kpoint-1][iband-1]
            dic_band_overlap[iband].append([ratio, wavecar_mult(psi_i, psi_f)])

    for iband in dic_band_overlap.keys():
        dic_band_overlap[iband] = np.array(dic_band_overlap[iband])

    # Compute wifs or gradients of overlaps with respect to dQ
    list_overlap = np.zeros((num_bands, num_wave+1))
    list_wif = [] 
    for j, (iband, elem) in enumerate(dic_band_overlap.items()):
        overlaps = elem.T[1]
        list_overlap[j,:-1] = overlaps
        list_overlap[j,-1] = overlaps[0]
        gradient_val = dE[j] * np.mean(np.abs(np.gradient(list_overlap[j, :], Q)))
        list_wif.append((iband, gradient_val))
    ...
    return dE[-1], dic_band_overlap, dic_wif, ixband_wifmax, wif

''' Functions to determine Wif from WSWQ: '''

def get_WSWQ(fname: str) -> dict:
    """Read the WSWQ file from VASP (overlap of wavefunctions) 
    -fname: path/to/WSWQ file
    """
...
def calc_wif_from_WSWQ(dir_f, list_data2, folder_q0, ix_defect, ix_bandmin, ix_bandmax, dQ, spin):
    '''
    We calculate the Wif from the Electronic Wavefunction overlaps obtained from VASP
    For this module, we need the WSWQ file inside each ratio-%
    '''
    ...
    ''' Compute the eigenvalue differences '''
    ar_eig1_q0 = read_eig(folder_q0, 'vasp') #load eigenvalues
    def_eigen = ar_eig1_q0[kpoint-1, spin-1, ix_defect-1, 0] # Eigenvalue of defect state

    dE_eig = np.zeros(num_bands) # array to store the energy differences of bulk and defect states
    for i, iband in enumerate(range(ix_bandmin, ix_bandmax+1)):
        band_eig = ar_eig1_q0[kpoint-1, spin-1, iband-1, 0]
        dE_eig[i] = float(-band_eig + def_eigen)
    
    ''' Loop through dQ values and bands and extract overlaps from WSWQ '''
    dic_band_overlap = {iband: [] for iband in range(ix_bandmin, ix_bandmax+1)}
    Q = np.zeros(num_wave+1)
    ...
    for i, data in enumerate(list_data2):
        ...
        Q[i] = ratio*dQ  #convert to Q
        wswq = get_WSWQ(os.path.join(folder2, "WSWQ"))

        for j, iband in enumerate(range(ix_bandmin, ix_bandmax+1)):
            print("%sOverlap: %i  %.4f" % (indent*3, iband, ratio))
            overlap = np.sign(Q[i]) * np.abs(wswq[(spin, kpoint)][(iband, ix_defect)])
            dic_band_overlap[iband].append([ratio, overlap])
            # overlap_list.append(np.asarray([ratio, overlap]))
            # dic_band_overlap[iband] = np.asarray(overlap_list)
    ...
    list_overlap = np.zeros((num_bands, num_wave+1))
    list_wif = []
    nq = 3 # range for fitting overlap vs dQ
    for j, (iband, elem) in enumerate(dic_band_overlap.items()):
        overlaps = elem.T[1]
        list_overlap[j,:-1] = overlaps
        wif = dE_eig[j] * np.polyfit(Q[:nq], list_overlap[j,:nq], 1)[0]
        list_wif.append((iband, wif))
    ...
    return dE_eig[-1], dic_band_overlap, dic_wif, ixband_wifmax, wif


# def calc_wif_vasp(dir_i, dir_f, ix_defect, ix_bandmin, ix_bandmax, dQ, de=None, spinname="down"):
#     ''' Calculate wifs from WAVECAR '''
#     if (ix_bandmin > ix_bandmax):
#         raise ValueError("Band max must be equal or larger than band min")
...
# # Compute dE
#     # Extract the folder of q0 depending on the program used.
#     spin = {"up": 1, "down": 2}[spinname]
#     if prog == "qe":
#         folder_q0 = get_save_folder(os.path.join(dir_f, get_ratio_folder(0)))
#     elif prog == "vasp":
#         folder_q0 = os.path.join(dir_f, get_ratio_folder(0))
#     ar_eig1_q0 = read_eig(folder_q0, prog)    
...
#     num_wave = len(list_data2)
#     num_bands = len(list(range(ix_bandmin, ix_bandmax+1)))
#     deig = np.zeros(num_bands)
#     Q = np.zeros(num_wave+1)
#     list_overlap = np.zeros((num_bands, num_wave+1))

#     '''
#     Compute eigenvalue differences and the overlap for dQ=0
#     '''
#     for i, iband in enumerate(range(ix_bandmin, ix_bandmax+1)):
#         if initial_wavecar.spin == 2:
#             psi_f = initial_wavecar.coeffs[spin-1][kpoint-1][iband-1]
#             deig[i] = initial_wavecar.band_energy[spin-1][kpoint-1][iband-1][0] - initial_wavecar.band_energy[spin-1][kpoint-1][ix_defect-1][0]
#         else:
#             psi_f = initial_wavecar.coeffs[kpoint-1][iband-1]
#             deig[i] = initial_wavecar.band_energy[kpoint-1][iband-1][0] - initial_wavecar.band_energy[kpoint-1][ix_defect-1][0]
#         list_overlap[i, num_wave] = wavecar_mult(psi_i, psi_f)
#     deig = np.abs(deig)
#     print("dQ=0: ")
#     print(list_overlap)
...
#         # (vecR, pos_i), _ = read_cell_and_pos_auto(os.path.join(folder2, "CONTCAR"))
#         # dQ,_,_,_ = calc_dQ(vecR, list_data2[0]["pos"], list_data2[i]["pos"])
#         # Q[i] = dQ
#         # print(dQ)
...
#             else:
#                 psi_f = final_wavecar.coeffs[kpoint-1][iband-1]
#             list_overlap[j, i] = wavecar_mult(psi_i, psi_f)
#             dic_band_overlap[iband].append([ratio, list_overlap[j, i]])

#     print(dic_band_overlap)

def calc_wif_qe(dir_f, list_data2, folder_q0, ix_defect, ix_bandmin, ix_bandmax, dQ, spin):
    
    ar_eig1_q0 = read_eig(folder_q0, 'qe')
...
#       print("Band %i dS/dQ %.2e Wif %.2e" % (iband, p[0], p[0] * dE))
    ...
    return dE, dic_band_overlap, dic_wif, ixband_wifmax, wif
