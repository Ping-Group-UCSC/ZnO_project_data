...
#   <Block Root  S    Ms  | HSOC |  Block Root  S    Ms>
  ...
  #1A1->3A2
    ...
    if re.match(line,"0\s+ 4\s+ 1.0\s+"):
        print(line) 
    if f"{triplet_3E_r2} 0.0              {triplet_3E_r1} 0.0" in line:
        soc = get_soc(line)
        print("within 3E (m0): (e->2a1) root2 3E(ms=0) -> (e->2a1) root1 3E(ms=0), lambda z") #within 3E(ms=0), lambdaz
    if f"{triplet_3E_r2} 1.0              {triplet_3E_r1} 1.0" in line:
        soc = get_soc(line)
        print("within 3E (m1): (e->2a1) root2 3E(ms=+1) -> (e->2a1) root1 3E(ms=+1), lambda z") #within 3E(ms=+1), lambdaz
    if f"{triplet_3E_r2}-1.0              {triplet_3E_r1} -1.0" in line:
        soc = get_soc(line)
        print("within 3E (m-1): (e->2a1) root2 3E(ms=-1) -> (e->2a1) root1 3E(ms=-1), lambda z") #within 3E(ms=-1), lambdaz
