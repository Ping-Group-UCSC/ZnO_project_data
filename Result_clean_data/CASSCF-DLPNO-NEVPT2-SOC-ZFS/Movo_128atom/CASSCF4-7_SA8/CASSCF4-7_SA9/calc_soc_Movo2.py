...
cwd = os.getcwd()
f = open(os.path.join(cwd, "SOC_result.txt"), "r")
lines = f.readlines()
...
#regex version
t_3A2='\s+0\s+0\s+1.0\s+'
t_3E_1 = '\s+0\s+3\s+1.0\s+'
t_3E_2 = '\s+0\s+4\s+1.0\s+'
s_1E_1 = '\s+1\s+0\s+0.0\s+'
s_1E_2 = '\s+1\s+1\s+0.0\s+'
s_1A1 = '\s+1\s+2\s+0.0\s+'
...
#   <Block Root  S    Ms  | HSOC |  Block Root  S    Ms>
   ...
   #3E -> 1E
    if f"{singlet_1E_r1} 0.0              {triplet_3E_r1} 0.0" in line:
        soc = get_soc(line)
        print("(e->2a1) root1 3E(ms=0) -> 1E root1 (ms=0), lambda z= ",np.round(soc,3), "GHz")
    if f"{singlet_1E_r1} 0.0              {triplet_3E_r2} 0.0" in line:
        soc = get_soc(line)
        print("(e->2a1) root2 3E(ms=0) -> 1E root1 (ms=0), lambda z= ",np.round(soc,3), "GHz")
    if f"{singlet_1E_r2} 0.0              {triplet_3E_r1} 0.0" in line:
        soc = get_soc(line)
        print("(e->2a1) root1 3E(ms=0) -> 1E root2 (ms=0), lambda z= ",np.round(soc,3), "GHz")
    if f"{singlet_1E_r2} 0.0              {triplet_3E_r2} 0.0" in line:
        soc = get_soc(line)
        print("(e->2a1) root2 3E(ms=0) -> 1E root2 (ms=0), lambda z= ",np.round(soc,3), "GHz")


    if f"{singlet_1E_r1} 0.0              {triplet_3E_r1} 1.0" in line:
        soc = get_soc(line)
        print("(e->2a1) root1 3E(ms=0) -> 1E root1 (ms=1), lambda x= ",np.round(soc,3), "GHz")
    if f"{singlet_1E_r1} 0.0              {triplet_3E_r2} 1.0" in line:
        soc = get_soc(line)
        print("(e->2a1) root2 3E(ms=0) -> 1E root1 (ms=1), lambda x= ",np.round(soc,3), "GHz")
    if f"{singlet_1E_r2} 0.0              {triplet_3E_r1} 1.0" in line:
        soc = get_soc(line)
        print("(e->2a1) root1 3E(ms=0) -> 1E root2 (ms=1), lambda x= ",np.round(soc,3), "GHz")
    if f"{singlet_1E_r2} 0.0              {triplet_3E_r2} 1.0" in line:
        soc = get_soc(line)
        print("(e->2a1) root2 3E(ms=0) -> 1E root2 (ms=1), lambda x= ",np.round(soc,3), "GHz")

    if f"{singlet_1E_r1} 0.0              {triplet_3E_r1}-1.0" in line:
        soc = get_soc(line)
        print("(e->2a1) root1 3E(ms=0) -> 1E root1 (ms=-1), lambda y= ",np.round(soc,3), "GHz")
    if f"{singlet_1E_r1} 0.0              {triplet_3E_r2}-1.0" in line:
        soc = get_soc(line)
        print("(e->2a1) root2 3E(ms=0) -> 1E root1 (ms=-1), lambda y= ",np.round(soc,3), "GHz")
    if f"{singlet_1E_r2} 0.0              {triplet_3E_r1}-1.0" in line:
        soc = get_soc(line)
        print("(e->2a1) root1 3E(ms=0) -> 1E root2 (ms=-1), lambda y= ",np.round(soc,3), "GHz")
    if f"{singlet_1E_r2} 0.0              {triplet_3E_r2} 1.0" in line:
        soc = get_soc(line)
        print("(e->2a1) root2 3E(ms=0) -> 1E root2 (ms=-1), lambda y= ",np.round(soc,3), "GHz")
  #1A1->3A2
    if f"{singlet_1A1} 0.0              {triplet_3A2} 0.0" in line:
        soc = get_soc(line)
        print("1A1 (ms=0) -> 3A2(ms=0), lambda z= ",np.round(soc,3), "GHz")
    if f"{singlet_1A1} 0.0              {triplet_3A2} 1.0" in line:
        soc = get_soc(line)
        print("1A1 (ms=0) -> 3A2(ms=1) -> 1A1(ms=0), lambda x= ",np.round(soc,3), "GHz")
    if f"{singlet_1A1} 0.0              {triplet_3A2}-1.0" in line:
        soc = get_soc(line)
        print("1A1 (ms=0) -> 3A2(ms=-1) -> 1A1(ms=0), lambda y= ",np.round(soc,3), "GHz")


    # print the 3E-3E ZFS splitting:
    if re.match(f"{t_3E_2}0.0 \s+ {t_3E_1}0.0",line):
        soc = get_soc(line)
        print("within 3E ms=0, lambda z = ",np.round(soc,3), "GHz") 
    if re.match(f"{t_3E_2} 1.0 \s+ {t_3E_1} 1.0",line):
        soc = get_soc(line)
        print("within 3E ms=1, lambda z = ",np.round(soc,3), "GHz")
    if re.match(f"{t_3E_2}-1.0 \s+ {t_3E_1}-1.0",line):
        soc = get_soc(line)
        print("within 3E ms=-1, lambda z = ",np.round(soc,3), "GHz")
    # between +-1: 
    ...
    if re.match(f"{t_3E_2}1.0 \s+ {t_3E_1}-1.0",line):
        soc = get_soc(line)
        print("within 3E ms=+-1, lambda z = ",np.round(soc,3), "GHz")
    if re.match(f"{t_3E_2}0.0 \s+ {t_3E_1}1.0",line):
        soc = get_soc(line)
        print("within 3E ms=0->1, lambda z = ",np.round(soc,3), "GHz")
