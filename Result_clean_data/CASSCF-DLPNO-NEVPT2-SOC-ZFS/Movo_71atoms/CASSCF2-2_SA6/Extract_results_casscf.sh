...
# Extract part between "ZERO-FIELD SPLITTING" and "--------------------------------"
awk '/ZERO-FIELD SPLITTING/,/Storing geometry in DTensor/' "$input_file" > zfs_result.txt

# Extract SSC contribution:
awk '/SSC CONTRIBUTION/,/Storing geometry in DTensor/' "$input_file" > zfs_ssc_result.txt
